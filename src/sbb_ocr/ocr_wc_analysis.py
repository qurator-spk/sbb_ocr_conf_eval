...
def plot_histogram(ax, data, weights, bins, xlabel, ylabel, color, histogram_info):
    ...
    binned_data = pd.cut(
        data, bins=bins, right=True, include_lowest=True
    )  # include_lowest ensures 0.0 is captured # type: ignore
    ...
    if weights is not None:
        ...
        bin_errors = df.groupby("bin", observed=False)["weight"].apply(
            lambda w: np.sqrt(np.sum(w**2))
        )
    else:
...
def plot_density(ax, data, weights, xlabel, ylabel, density_color, stats_collector):
    ...
    try:
        ...
        q25, q50, q75 = (
            weighted_percentile(data, weights, [25, 50, 75])
            if is_weighted
        ...
        ax.axvline(
            q75, color="black", linestyle="dashdot", linewidth=1, label="Q3: 75%"
        )
        ...
        if is_weighted:
            ...
            stats_collector[unweighted_header] = pd.DataFrame(uw_stats_dict)[
                "Value"
            ].values
...
def create_plots(
    results_df, weights_word, weights_textline, plot_file, histogram_info, general_title
):
    density_stats = {}
    ...
    plot_histogram(
        axs[0, 2],
        results_df["standard_deviation_word"],
        weights_word,
        bins,
    ...
    plot_histogram(
        axs[1, 2],
        results_df["standard_deviation_textline"],
        weights_textline,
        bins,
...
def create_publication_count_horizontal_barplot(
    labels, counts, title, ylabel, filename, fontsize_scale=1.0
):
...
def create_weighted_means_dates_barplot(
    plot_df, results_df, groupby_col, label_col, title, filename
):
    word_errors = []
    textline_errors = []
    for label in plot_df[label_col]:
        ...
        word_se = (
            weighted_standard_error_of_the_mean(
                group_ppns["mean_word"], group_ppns["weight_word"]
            )
            if len(group_ppns) > 1
            else 0
        )
        textline_se = (
            weighted_standard_error_of_the_mean(
                group_ppns["mean_textline"], group_ppns["weight_textline"]
            )
            if len(group_ppns) > 1
            else 0
        )
    ...
    bars2 = ax.barh(
        y - height / 2,
        plot_df["Weighted_Mean_Textline"],
        height,
        xerr=textline_errors,
        capsize=5,
        label="Textline Confidence",
        color="salmon",
        alpha=0.7,
    ...
    df_save = plot_df[
        [label_col, "Weighted_Mean_Word", "Weighted_Mean_Textline"]
    ].copy()
...
def create_weighted_means_genre_and_subgenre_barplot(
    plot_df, label_col, title, filename, word_errors, textline_errors
):
    ...
    labels = [
        f"{label} (n={count})"
        for label, count in zip(plot_df[label_col], plot_df["Count"])
    ]
...
def process_weighted_means(data_dict, label_name, filename_prefix, counts_dict):
    ...
    for label, data in data_dict.items():
        ...
        if count < 20:
            ...
            word_se = (
                weighted_standard_error_of_the_mean(
                    data["mean_word"], data["weight_word"]
                )
                if len(data["mean_word"]) > 1
                else 0
            )
            textline_se = (
                weighted_standard_error_of_the_mean(
                    data["mean_textline"], data["weight_textline"]
                )
                if len(data["mean_textline"]) > 1
                else 0
            )
    ...
    if other_data["mean_word"]:
        ...
        wm_textline = weighted_mean(
            other_data["mean_textline"], other_data["weight_textline"]
        )
        word_se = weighted_standard_error_of_the_mean(
            other_data["mean_word"], other_data["weight_word"]
        )
        textline_se = weighted_standard_error_of_the_mean(
            other_data["mean_textline"], other_data["weight_textline"]
        )
    ...
    for idx, row in df_save.iterrows():
        ...
        if label == "Other (sum of all n < 20)":
            ...
            df_save.at[idx, "weight_textline"] = sum(
                data_dict[label]["weight_textline"]
            )
...
def genre_evaluation(metadata_df, results_df, use_threshold=False):
    ...
    for ppn in matching_ppn_mods:
        ...
        if is_ppn_page_mode:
            ...
            ppn_results = (
                pd.DataFrame([ppn_to_results.get(ppn)])
                if ppn_to_results.get(ppn) is not None
                else pd.DataFrame()
            )
        ...
        for genre_raw in current_genres_raw_list:
            if not genre_raw:
                continue
            genres_json = (
                genre_raw.replace("{", "[").replace("}", "]").replace("'", '"')
            )
            ...
            for genre, subgenre in genres_with_subgenres:
                if subgenre:
                    ...
                    genre_subgenre_weighted_data[combo_key]["mean_word"].extend(
                        ppn_results["mean_word"].tolist()
                    )
                    genre_subgenre_weighted_data[combo_key]["weight_word"].extend(
                        ppn_results["weight_word"].tolist()
                    )
                    genre_subgenre_weighted_data[combo_key]["mean_textline"].extend(
                        ppn_results["mean_textline"].tolist()
                    )
                    genre_subgenre_weighted_data[combo_key]["weight_textline"].extend(
                        ppn_results["weight_textline"].tolist()
                    )
            ...
            for sub in subgenres:
                ...
                subgenre_weighted_data[sub]["mean_word"].extend(
                    ppn_results["mean_word"].tolist()
                )
                subgenre_weighted_data[sub]["weight_word"].extend(
                    ppn_results["weight_word"].tolist()
                )
                subgenre_weighted_data[sub]["mean_textline"].extend(
                    ppn_results["mean_textline"].tolist()
                )
                subgenre_weighted_data[sub]["weight_textline"].extend(
                    ppn_results["weight_textline"].tolist()
                )
            ...
            for genre in set(genres):
                ...
                genre_weighted_data[genre]["mean_word"].extend(
                    ppn_results["mean_word"].tolist()
                )
                genre_weighted_data[genre]["weight_word"].extend(
                    ppn_results["weight_word"].tolist()
                )
                genre_weighted_data[genre]["mean_textline"].extend(
                    ppn_results["mean_textline"].tolist()
                )
                genre_weighted_data[genre]["weight_textline"].extend(
                    ppn_results["weight_textline"].tolist()
                )
    ...
    genre_counts_df_sorted = genre_counts_df.sort_values(
        by=["Count", "Genre"], ascending=[False, True]
    )
    ...
    subgenre_counts_df = pd.DataFrame(
        list(subgenre_counts.items()), columns=["Subgenre", "Count"]
    )
    subgenre_counts_df_sorted = subgenre_counts_df.sort_values(
        by=["Count", "Subgenre"], ascending=[False, True]
    )
    ...
    if not subgenre_counts_df.empty:
        ...
        for genre in sorted(genre_to_subgenre_counts.keys()):
            ...
            subgenre_str = "; ".join(
                [f"{sub} ({count})" for sub, count in sorted_items]
            )
            genre_subgenre_summary.append((genre, subgenre_str))

        genre_subgenre_df = pd.DataFrame(
            genre_subgenre_summary, columns=["Genre", "Subgenres (with counts)"]
        )
    ...
    if not filtered_genre_counts:
        ...
        process_weighted_means(
            genre_weighted_data,
            label_name="Genre",
            filename_prefix="genre",
            counts_dict=genre_counts,
        )
        ...
        if len(subgenre_counts) >= 1:
            process_weighted_means(
                subgenre_weighted_data,
                label_name="Subgenre",
                filename_prefix="subgenre",
                counts_dict=subgenre_counts,
            )
            ...
            for genre in sorted(genre_to_subgenre_counts.keys()):
                ...
                for subgenre, count in subgenre_items:
                    flattened_data.append(
                        {"label": f"{genre}: {subgenre}", "count": count}
                    )
            ...
            if flattened_labels and flattened_counts:
                ...
                genre_subgenre_counts = {
                    item["label"]: item["count"] for item in flattened_data
                }  # NEW
...
def dates_evaluation(metadata_df, results_df):
    ...
    metadata_df.loc[:, "publication_date"] = pd.to_numeric(
        metadata_df["publication_date"], errors="coerce"
    )
    try:
        ...
        year_counts = (
            metadata_df["publication_date"]
            .value_counts()
            .reindex(full_year_range, fill_value=0)
        )
        ...
        year_counts_df.to_csv(
            f"date_range_{min_year}-{max_year}_publications_per_year.csv", index=False
        )
        ...
        for year, group in grouped:
            ...
            if len(subgroup) > 0:
                ...
                wm_textline = weighted_mean(
                    subgroup["mean_textline"], subgroup["weight_textline"]
                )
            else:
                wm_word = np.nan
                wm_textline = np.nan
        ...
        results_df = results_df.merge(
            metadata_df[["PPN", "publication_date"]],
            how="left",
            left_on="ppn",
            right_on="PPN",
        )
...
def languages_evaluation(metadata_df, results_df):
    ...
    for ppn in matching_ppns:
        ...
        languages = sorted(
            set(lang.strip() for lang in lang_string.split(",") if lang.strip())
        )
        if not languages:
            continue
        ...
        # Count publication
        language_combination_counts[normalized_lang] = (
            language_combination_counts.get(normalized_lang, 0) + 1
        )
        ...
        language_combination_data[normalized_lang]["mean_word"].extend(
            ppn_results["mean_word"].tolist()
        )
        language_combination_data[normalized_lang]["weight_word"].extend(
            ppn_results["weight_word"].tolist()
        )
        language_combination_data[normalized_lang]["mean_textline"].extend(
            ppn_results["mean_textline"].tolist()
        )
        language_combination_data[normalized_lang]["weight_textline"].extend(
            ppn_results["weight_textline"].tolist()
        )
    ...
    sorted_lang_counts = sorted(
        language_combination_counts.items(), key=lambda x: (-x[1], x[0])
    )
    lang_combos, lang_counts = (
        zip(*sorted_lang_counts) if sorted_lang_counts else ([], [])
    )

    language_counts_df = pd.DataFrame(
        list(language_combination_counts.items()),
        columns=["Language_Combination", "Count"],
    )
    language_counts_df_sorted = language_counts_df.sort_values(
        by=["Count", "Language_Combination"], ascending=[False, True]
    )
    language_counts_df_sorted.to_csv(
        "language_combination_publications.csv", index=False
    )
...
def create_weights_and_num_pages_barplot(
    data_pairs, titles, xlabels, ylabels, filename, errors_pairs=None
):
...
def compute_unweighted_stats(df, bin_col, value_col):
    ...
    for label, group in df.groupby(bin_col, observed=False):
        ...
        stats_data.append(
            {"Bin": str(label), "Mean": mean, "Error": sem, "Count": count}
        )
...
def weights_evaluation(results_df):
    ...
    results_df["textline_bin"] = pd.cut(
        results_df["weight_textline"], bins=textline_bins
    )

    word_labels, word_means, word_errors, word_stats_df = compute_unweighted_stats(
        results_df, "word_bin", "mean_word"
    )
    textline_labels, textline_means, textline_errors, textline_stats_df = (
        compute_unweighted_stats(results_df, "textline_bin", "mean_textline")
    )

    word_stats_df.columns = [
        "Word_Count_Bin",
        "Mean_Word_Confidence",
        "Word_Confidence_Error",
        "Count",
    ]
    textline_stats_df.columns = [
        "Textline_Count_Bin",
        "Mean_Textline_Confidence",
        "Textline_Confidence_Error",
        "Count",
    ]
...
def num_pages_evaluation(results_df):
    ...
    for bin_label, group in grouped:
        if len(group) > 0:
            ...
            wm_textline = weighted_mean(
                group["mean_textline"], group["weight_textline"]
            )
            word_se = weighted_standard_error_of_the_mean(
                group["mean_word"], group["weight_word"]
            )
            textline_se = weighted_standard_error_of_the_mean(
                group["mean_textline"], group["weight_textline"]
            )
    ...
    create_weights_and_num_pages_barplot(
        ...
        titles=[
            "Weighted Mean Word Confidence by Page Count",
            "Weighted Mean Textline Confidence by Page Count",
        ],
        ...
        for ppn_name in os.listdir():
            ...
            if os.path.isdir(full_path) and ppn_name.startswith("PPN"):
                ...
                if not os.path.exists(gt_dir) or not os.path.exists(ocr_dir):
                    logging.info(
                        f"Missing subdirectories in {ppn_name}: {gt_dir} or {ocr_dir}"
                    )
                    os.chdir(parent_dir)
                    progbar.update(1)
                    return
                ...
                command_string = (
                    f"ocrd-dinglehopper -I {gt_dir},{ocr_dir} -O {report_dir}"
                )
                ...
                try:
                    ...
                    logging.info(
                        f"Failed to run command in {ppn_name}. Exit code: {e.returncode}, Error: {e.stderr}"
                    )
                    ...
                    for json_file in os.listdir(eval_dir):
                        if json_file.endswith(".json"):
                            ...
                            try:
                                ...
                                logging.info(
                                    f"Error reading JSON file {json_path}: {e}"
                                )
                                continue
                else:
                    logging.info(
                        f"Evaluation directory not found for {ppn_name}: {eval_dir}"
                    )
...
def merge_csv(conf_df, error_rates_df, wcwer_filename):
    ...
    try:
        ...
        ppn_conf_df = ppn_conf_df[
            ppn_conf_df["ppn_page"].isin(ppn_error_rates_df["ppn_page"])
        ]
...
def calculate_regression_statistics(X, y, wcwer_df):
    ...
    var_matrix_linear = mse_linear * np.linalg.inv(
    ...
    try:
        ...
        p_values_poly = [
            2 * (1 - stats.t.cdf(abs(t), degrees_of_freedom)) for t in t_stats
        ]
    ...
    se_pred_poly = np.sqrt(
        ...
        * np.diag(
            X_smooth_poly
            @ np.linalg.inv(X_poly_train.T @ X_poly_train)
            @ X_smooth_poly.T
        )
    )
...
def plot_wer_vs_wc(wcwer_csv, plot_filename):
    ...
    try:
        ...
        missing_columns = [
            col for col in required_columns if col not in wcwer_df.columns
        ]
        if missing_columns:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                hovertemplate=(
                    ...
                    "Mean Word Confidence: %{x:.3f}<br>" + "WER: %{y:.3f}"
                ),
        ...
        fig.add_trace(
            go.Scatter(
                ...
                name="Test Points",
                marker=dict(
                    size=10, color="orange", line=dict(width=2, color="DarkSlateGrey")
                ),
                hovertemplate=(
                    "PPN Page: %{customdata}<br>"
                    "Mean Word Confidence: %{x:.3f}<br>" + "WER: %{y:.3f}"
        ...
        # Update layout
        fig.update_layout(
            title=dict(text=f"WER vs WC", x=0.5, xanchor="center"),
        ...
        # Add scatter plots
        plt.scatter(
            X_train, y_train, color="blue", marker="x", s=100, label="Training Points"
        )
        plt.scatter(
            X_test, y_test, color="darkorange", marker="x", s=100, label="Test Points"
        )
        ...
        plt.xlabel("Mean Word Confidence Score (WC)", fontsize=23)
        plt.ylabel("Word Error Rate (WER)", fontsize=23)
        plt.title("WER vs WC", fontsize=22)
...
def merge_layout_data(
    layout_csv: str, results_df: pd.DataFrame, metadata_df: pd.DataFrame
) -> pd.DataFrame:
    ...
    merged_df = pd.merge(layout_df, results_df, on="ppn_page", how="inner")
    ...
    final_df = pd.merge(
        merged_df, metadata_df, left_on="ppn", right_on="PPN", how="inner"
    )
    ...
    cols = ["PPN_Page", "Kategorien"] + [
        col for col in final_df.columns if col not in ["PPN_Page", "Kategorien"]
    ]
...
def generate_dataframes(
    ...
    with tqdm(total=len(csv_files)) as progbar:
        for csv_file in csv_files:
            ...
            try:
                with load_csv(csv_file) as rows:
                    for i, row in enumerate(rows):
                        ...
                        mean_textline, median_textline, standard_deviation_textline = (
                            statistics(textline_confs)
                        )
                        mean_word, median_word, standard_deviation_word = statistics(
                            word_confs
                        )
                        all_results.append(
                            [
                ...
                exc = ValueError(
                    f"CSV error: {e} in file: {csv_file}. \nIncrease the CSV field size limit!"
                )
                logging.exception(exc)
                raise exc
            progbar.update(1)
    ...
    if aggregate_mode == "ppn":
        ...
        for ppn, group in grouped:
            ...
            median_word = weighted_percentile(group["median_word"], word_weights, [50])[
                0
            ]
            ...
            median_textline = weighted_percentile(
                group["median_textline"], textline_weights, [50]
            )[0]
            std_textline = weighted_std(
                group["standard_deviation_textline"], textline_weights
            )

            total_word_weight = word_weights.sum()
            total_textline_weight = textline_weights.sum()

            num_pages = page_counts.loc[page_counts["ppn"] == ppn, "num_pages"].values[
                0
            ]
    ...
    metadata_df = pd.DataFrame(
        load_csv_to_list(metadata_csv)[1:],
        columns=["PPN", "genre", "publication_date", "language"],
    )
    ...
    if check_value_errors:
        ...
        logging.info(
            f"Number of PPN_PAGEs excluded because of a ValueError: {ppn_page_counts.sum()}"
        )
    ...
    metadata_df.loc[
        metadata_df["publication_date"].isin(["", "18XX"]), "publication_date"
    ] = "2025"
    ...
    metadata_df["genre"] = metadata_df["genre"].apply(
        lambda genre: "{"
        + genre.strip().strip("{ }").replace("  / ", "', '").replace(" / ", "', '")
        + "}"
    )
    ...
    metadata_df["language"] = metadata_df["language"].apply(
        lambda lang: lang.replace("$b", ", ").replace("de", "ger").strip()
    )
    ...
    if check_duplicates:
        ...
        logging.info(
            f"Number of PPN_PAGEs with a single occurrence: {singles_summary.sum()}"
        )

        duplicates_summary = ppn_page_counts[ppn_page_counts > 1]
        num_unique_duplicates = len(duplicates_summary)
        logging.info(
            f"Number of PPN_PAGEs with multiple occurrences: {num_unique_duplicates}"
        )
        ...
        filtered_df = filtered_df.merge(
            metadata_filtered, left_on="ppn", right_on="PPN"
        )
...
def plot_everything(
    ...
    if search_date is not None:  # "is not None" enables zero as input
        results_df = results_df[
            results_df["ppn"].isin(
                metadata_df.loc[
                    (metadata_df["publication_date"].astype(int) == search_date), "PPN"
                ]
            )
        ]  # type: ignore
    ...
    if search_weight_textline is not None:
        results_df = results_df[
            (results_df["weight_textline"] == search_weight_textline)
        ]
    ...
    if mean_textline_confs_range:
        results_df = results_df.sort_values(by="mean_textline", ascending=True)
        results_df = filter_range(
            results_df, "mean_textline", mean_textline_confs_range
        )
    ...
    if search_genre:
        ...
        results_df = results_df[
            results_df["ppn"].isin(
                metadata_df.loc[
                    metadata_df["genre"].str.match(pattern, na=False), "PPN"
                ]
            )
        ]
    ...
    if search_subgenre:
        ...
        results_df = results_df[
            results_df["ppn"].isin(
                metadata_df.loc[
                    metadata_df["genre"].str.contains(pattern, na=False), "PPN"
                ]
            )
        ]
    ...
    if search_language:
        ...
        results_df = results_df[
            results_df["ppn"].isin(
                metadata_df.loc[
                    metadata_df["language"].str.contains(pattern, na=False), "PPN"
                ]
            )
        ]
    ...
    if use_top_ppns_word:
        results_df = results_df[
            ((results_df["mean_word"] >= 0.95) & (results_df["mean_word"] <= 1.0))
        ]
        ...
        results_df = results_df[
            ((results_df["mean_word"] >= 0.0) & (results_df["mean_word"] <= 0.05))
        ]
    ...
    if use_top_ppns_textline:
        results_df = results_df[
            (
                (results_df["mean_textline"] >= 0.95)
                & (results_df["mean_textline"] <= 1.0)
            )
        ]
        ...
        results_df = results_df[
            (
                (results_df["mean_textline"] >= 0.0)
                & (results_df["mean_textline"] <= 0.05)
            )
        ]
        results_df = results_df.sort_values(by="mean_textline", ascending=True)
    ...
    if parent_dir and conf_filename:
        get_ppn_subdirectory_names(
            results_df=results_df, parent_dir=parent_dir, conf_filename=conf_filename
        )
    ...
    if show_results:
        if len(results_df_unique) > 0:
            ...
            metadata_filtered = metadata_df[
                ["PPN", "publication_date", "genre", "language"]
            ]
            filtered_results_df = filtered_results_df.merge(
                metadata_filtered, left_on="ppn", right_on="PPN"
            )
    ...
    plot_file_weighted = (
        plot_file.split(".")[0] + "_weighted." + plot_file.split(".")[1]
    )
...
def evaluate_everything(
    ...
    if parent_dir and gt_dir and ocr_dir and report_dir:
        use_dinglehopper(
            parent_dir=parent_dir, gt_dir=gt_dir, ocr_dir=ocr_dir, report_dir=report_dir
        )

    if parent_dir_error and report_dir_error and error_rates_filename:
        generate_error_rates(
            parent_dir_error=parent_dir_error,
            report_dir_error=report_dir_error,
            error_rates_filename=error_rates_filename,
        )
